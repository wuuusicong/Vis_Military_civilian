...
class WordCloud2 extends Component {
    ...
    componentDidMount() {
        ...
        let data =  this.props.data.map((item,index)=>{
            return {"x":item["text"],"value":item["value"],label:this.props.text,text:item["text"]}
        });
        const dv = new DataView().source(data);
        const range = dv.range('value');
        const min = range[0];
        ...
        if(this.state.data2!==undefined){
            return (
                <div id={this.props.id}>
                    <Chart width="258" height="93" data={this.state.data2} scale={scale} padding={[0]}>
                        <Tooltip showTitle={false}/>
                        <Coord type="rect" direction="TL"/>
                        <Point position="x*y" color={["label", ["#000000","#4e8bdc","#998ec3"]]} shape="cloud" tooltip={this.state.text} fill="#fff"/>
                    </Chart>
                </div>
            )
        }else {
