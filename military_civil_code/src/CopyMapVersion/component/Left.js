...
class containerLeft extends  Component {
    constructor(props){
        ...
        this.state = {data:[{"w2":-45},{"w3":45},{"w4":-135},{"w5":135}],
            fourAreaData,fourText:["科创区","游仙区","涪城区","高新区"],copyFourAreaData,flag};
        ...
        if(this.state.flag){
             listDiv = this.state.data.map((item,index)=>{
                if(this.state.copyFourAreaData[index].length===0){return (
                    ...
                    <div className={Object.keys(item)[0]+'text'}>{this.state.fourText[index]}</div>
                    ...
                    <div key={index} className="detailWord"><div className={Object.keys(item)[0]+'text'}>
                        {this.state.fourText[index]}</div><WordCloud2 id={Object.keys(item)[0]}
                                                                     text={this.state.fourText[index]} data={this.state.copyFourAreaData[index]}
                ...
                listDiv = this.state.data.map((item,index)=>{
                    return (
                        <div key={index} className="detailWord"><div className={Object.keys(item)[0]+'text'}>
                            {this.state.fourText[index]}</div><WordCloud id={Object.keys(item)[0]}
                                  text={this.state.fourText[index]} data={this.state.copyFourAreaData[index]}
                                  rotate={item[Object.keys(item)[0]]}/></div>)
})}
        console.log(listDiv);
        ...
        return (
            <div id= "containerLeft" style={style}>
                ...
                <div id="containerLeftDown">
                    <div  id="fourWord">
                        {listDiv}
                        {this.state.flag?<div id="returnDiv" onClick={this.backMain}>概览</div>:null}
